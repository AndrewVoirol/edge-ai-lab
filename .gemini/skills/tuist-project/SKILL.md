...
- `Derived/` directory (generated Info.plist files, etc.)
...
**Symptom:** Missing `Info.plist`, `Bundle+GemmaEdgeGallery.swift` not found

**Fix:** These files live in `Derived/` which is created by `tuist generate`. If missing:
```bash
tuist generate
```
...
├── Derived/                           # Generated by Tuist (Info.plists, etc.)
