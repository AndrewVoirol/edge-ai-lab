...
final class ConversationViewModelSamplerTests: XCTestCase {
    ...
    @MainActor func testEmptySystemMessagePassedAsNil() async {
        ...
        XCTAssertNil(
            ...
            "Empty systemMessage with thinking disabled should be mapped to nil"
        )
    }

    // MARK: - Thinking Mode System Message
    ...
    @MainActor func testThinkingEnabledInjectsThinkToken() async {
        ...
        XCTAssertEqual(
            mockEngine.lastSystemMessage, "<|think|>",
            "Thinking enabled with no user system message should produce just <|think|>"
        )
    }
    ...
    @MainActor func testThinkingEnabledWithUserSystemMessage() async {
        ...
        XCTAssertEqual(
            mockEngine.lastSystemMessage, "<|think|>\nYou are a pirate.",
            "Thinking enabled with user system message should combine with newline"
        )
    }
