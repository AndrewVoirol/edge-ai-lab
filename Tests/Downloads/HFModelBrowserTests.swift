...
final class HFModelBrowserTests: XCTestCase {
    ...
    func test_hfModelBrowser_formatDetection_unknown() {
        ...
        let format = browser.detectFormat(model)
        XCTAssertEqual(format, .unknown, "Model without known format files or metadata should be unknown")
    }
    ...
    func test_hfModelBrowser_formatDetection_noSiblings() {
        ...
        let format = browser.detectFormat(model)
        XCTAssertEqual(format, .unknown, "Model with nil siblings and no litert metadata should be unknown")
    }
