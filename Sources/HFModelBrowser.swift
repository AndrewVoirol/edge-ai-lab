...
final class HFModelBrowser {
    ...
    /// Detect the model format from the repository's file listing.
    ///
    /// Inspects the `siblings` array for known file patterns:
    /// - `.litertlm` extension → LiteRT-LM format
    /// - `config.json` + `*.safetensors` → MLX format
    ///
    /// - Parameter model: The model info (must have populated `siblings` for accurate detection).
    /// - Returns: The detected `HFModelFormat`.
    func detectFormat(_ model: HFModelInfo) -> HFModelFormat {
        ...
        // Check for LiteRT-LM format: any .litertlm file
