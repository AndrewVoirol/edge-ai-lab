...
struct ExperimentConfig: Codable, Sendable, Equatable {
    ...
    /// Create an `ExperimentConfig` from the current ViewModel state.
    @MainActor
    static func capture(from viewModel: ConversationViewModel) -> ExperimentConfig {
        ...
        let variant = Self.parseVariant(from: viewModel.activeModelURL?.lastPathComponent ?? "")

        return ExperimentConfig(
            modelName: viewModel.activeModelMetadata?.name ?? viewModel.activeModelURL?.lastPathComponent ?? "Unknown Model",
            modelFile: viewModel.activeModelURL?.lastPathComponent ?? "unknown",
            modelId: viewModel.activeModelMetadata?.modelId,
            architectureType: viewModel.activeModelMetadata?.architectureType,
            ...
            topP: viewModel.topP,
            seed: viewModel.seed,
            thinkingEnabled: viewModel.experimentalFlags.enableThinking,
            toolCallingEnabled: viewModel.experimentalFlags.enableToolCalling,
            agentSkillsEnabled: viewModel.experimentalFlags.enableAgentSkills,
            mtpEnabled: viewModel.experimentalFlags.enableSpeculativeDecoding ?? false,
            benchmarkEnabled: viewModel.experimentalFlags.enableBenchmark,
            systemMessage: viewModel.systemMessage.isEmpty ? nil : viewModel.systemMessage,
            createdAt: Date()
        )
    }
