...
- **mlx-swift CudaBuild plugin requires trust.** mlx-swift 0.31.6+ includes a CudaBuild SPM build plugin for Linux CUDA support. Xcode validates ALL plugins regardless of target platform, so macOS/iOS builds fail with "Missing package product" errors until the plugin is trusted. Fixes: (a) In Xcode IDE, click "Trust & Enable" when prompted on first build. (b) For headless/CI builds, use `-skipPackagePluginValidation`. (c) To permanently skip for the IDE: `defaults write com.apple.dt.Xcode IDESkipPackagePluginFingerprintValidatation -bool YES`. If you see "Missing package product 'LiteRTLM'" or similar after a fresh `tuist generate`, this is the cause — the packages resolved fine, but Xcode won't build until plugins are trusted.
